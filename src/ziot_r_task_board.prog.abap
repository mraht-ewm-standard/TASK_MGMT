  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS scan
      IMPORTING
        ...
        it_dev_objects   TYPE ziot_cl_package=>t_tadir
      RETURNING
  ...
  METHOD search.
    ...
    IF iv_incl_subpckg EQ abap_true.
      LOOP AT it_r_package ASSIGNING FIELD-SYMBOL(<ls_r_package>).
        DATA(lt_packages) = ziot_cl_package=>det_packages( iv_package = CONV #( <ls_r_package>-low ) ).
    ...
    DATA(lt_r_obj_type) = VALUE rseloption( ( sign = 'I' option = 'EQ' low = ziot_cl_package=>mc_dev_obj_type-clas )
                                            ( sign = 'I' option = 'EQ' low = ziot_cl_package=>mc_dev_obj_type-fugr )
                                            ( sign = 'I' option = 'EQ' low = ziot_cl_package=>mc_dev_obj_type-intf )
                                            ( sign = 'I' option = 'EQ' low = ziot_cl_package=>mc_dev_obj_type-prog ) ).
    DATA(lt_dev_objects) = ziot_cl_package=>get_dev_objects( iv_with_includes = abap_true
